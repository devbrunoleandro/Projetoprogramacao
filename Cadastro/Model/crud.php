...
class crud {    
 ...
 function inserir_Usuario($nome,$email,$telefone,$senha){
 

   $conn= conectar();   

   $pegaEmail = mysqli_query($conn, "SELECT * FROM tabela WHERE email = '$email'");	
   ...
   $sql = "INSERT INTO tabela (nome,email,telefone,senha) values('$nome','$email','$telefone','$senha')";
   ...
   function selecionar_Usuario($email,$senha){
        ...
        $result_usuario = "SELECT * FROM tabela  WHERE email= '$email' && senha = '$senha' LIMIT 1";
